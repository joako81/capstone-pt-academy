...
export function CourseItem(props) {
  ...
  const [showConfirm, setShoConfirm] = useState(false);
  const { accessToken } = useAuth();

  const onOpenCloseModal = () => setShowModal((prevState) => !prevState);
  const onOpenCloseConfirm = () => setShoConfirm((prevState) => !prevState);
    ...
    <>
      <div className="course-item">
        ...
        <div>
          ...
          <Button icon color="blue" onClick={openUpdateCourse}>
            <Icon name="pencil" />
          </Button>
      ...
      <Confirm
        ...
        onConfirm={onDelete}
        content={`Eliminiar el curso ${course.title}`}
        size="mini"
      />
    </>
  );
